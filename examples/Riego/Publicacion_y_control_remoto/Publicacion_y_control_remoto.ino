...
#define MSG_RIEGO_REMOTO F("Activado remotamente")
...
int iRiegoForzado=0; // Indica si el riego se ha activado desde fuera
...
void loop() {
  if(millis()-lUltimaMedida>lIntervaloMedida){ // Toca Medir
    ...
    showData();
    if(iRiegoForzado==1)    {   //Activa desde fuera
      Serial.println(MSG_RIEGO_REMOTO);
      lcd.setCursor(0,1);
      lcd.print(MSG_RIEGO_REMOTO);
      Ciao.println(MSG_RIEGO_REMOTO);
      enciendeRiego();
    } else {
